# 📊 Telco Customer Churn Analysis & Prediction

An end-to-end Machine Learning project that analyzes customer churn in the telecom industry and predicts whether a customer is likely to churn using classification models.

---

## 🌐 Live Demo

🔗 **Try the Live Application:** https://telco-churn-analysis-c66ns8v7eks7cpn3ewwwu7.streamlit.app

🔗 **GitHub Repository:** https://github.com/prangya-k-nayak/telco-churn-analysis

---

## 📸 Dashboard Preview

### 🏠 Home

![Home Dashboard](assets/home.png)

### 📊 Dataset

![Dataset](assets/dataset.png)

### 📈 EDA

![EDA](assets/eda.png)

### 🤖 Model Performance

![Model Performance](assets/model-performance.png)

### 🔮 Prediction

![Prediction](assets/prediction.png)

### ℹ️ About

![About](assets/about.png)

---

## 🚀 Features

- 📊 Dataset Overview
- 📈 Exploratory Data Analysis (EDA)
- 🤖 Model Performance Comparison
- 🔮 Customer Churn Prediction
- 📥 Download Dataset
- 🌐 Interactive Streamlit Dashboard

---

## 📂 Project Structure

```
telco-churn-analysis/
│
├── app/
│   ├── Home.py
│   └── pages/
│       ├── 1_Dataset.py
│       ├── 2_EDA.py
│       ├── 3_Model_Performance.py
│       ├── 4_Prediction.py
│       └── 5_About.py
│
├── data/
│   └── raw/
│
├── models/
│   ├── logistic_regression.pkl
│   ├── random_forest.pkl
│   ├── scaler.pkl
│   └── label_encoders.pkl
│
├── notebooks/
│
├── src/
│   ├── data/
│   └── models/
│
├── requirements.txt
└── README.md
```

---

## 📊 Dataset

IBM Telco Customer Churn Dataset

- 7,043 Customers
- 21 Features
- Binary Classification Problem

---

## 🛠 Technologies Used

- Python
- Pandas
- NumPy
- Scikit-learn
- Matplotlib
- Seaborn
- Plotly
- Streamlit

---

## 🤖 Machine Learning Models

- Logistic Regression
- Random Forest Classifier

---

## 📊 Model Performance

| Model | Accuracy |
|--------|---------:|
| Logistic Regression | 81.55% |
| Random Forest | 79.63% |

---

## ▶️ Run Locally

Clone the repository

```bash
git clone https://github.com/prangya-k-nayak/telco-churn-analysis.git
```

Install dependencies

```bash
pip install -r requirements.txt
```

Run Streamlit

```bash
streamlit run app/Home.py
```

---

## 📈 Results

The dashboard provides:

- Interactive EDA
- Model comparison
- Classification reports
- Customer churn prediction
- Prediction probability
- Business recommendations

---

## 🚀 Future Improvements

- Explainable AI (SHAP) for prediction interpretation
- Advanced dashboard UI and animations
- Customer segmentation and clustering
- PDF report generation
- Additional machine learning models
- Docker deployment

---

## 👩‍💻 Author

**Prangya Kumari Nayak**

MCA (Artificial Intelligence & Machine Learning)  
Jain (Deemed-to-be University)

📧 pragyakumarinayak173@gmail.com

🐙 GitHub: https://github.com/prangya-k-nayak